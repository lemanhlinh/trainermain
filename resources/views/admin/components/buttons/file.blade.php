<div class="modal-example">
    <button id="ckfinder-modal-{{ $name }}" type="button" class="btn btn-secondary">File</button>
    <div id="output-{{ $name }}">
        <input type="hidden" class="custom-file-input" id="{{ $name }}" name="{{ $name }}"
               value="{{ isset($src) ? $src : old( $name ) }}">
        @if(!empty($src))
            <a href="{{ asset($src) }}">Link file</a>
        @endif
    </div>
</div>
            ...
            onInit: function( finder ) {
                finder.on( 'files:choose', function( evt ) {
                    ...
                    output.innerHTML = '<input type="hidden" class="custom-file-input" id="{{ $name }}" name="{{ $name }}"\n' +
                        '               value="'+escapeHtml( file.getUrl() )+'"> <a href="'+escapeHtml( file.getUrl() )+'">Link file</a>';
                } );

                finder.on( 'file:choose:resizedImage', function( evt ) {
                    const output = document.getElementById( 'output-{{ $name }}' );
                    output.innerHTML = '<input type="hidden" class="custom-file-input" id="{{ $name }}" name="{{ $name }}"\n' +
                        '               value="'+escapeHtml( evt.data.resizedUrl )+'"><a href="'+escapeHtml( evt.data.resizedUrl )+'">Link file</a>';
                } );
            }
        } );
    };
